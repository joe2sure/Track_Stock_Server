...
export class StockService {
  ...
  async getStockLevels(
    ...
    const [data, total] = await Promise.all([
      StockLevel.find(filter)
        ...
        .skip(skip)
        .limit(limit)
        .lean() as unknown as IStockLevel[],
  ...
  async getProductStock(
    ...
    return StockLevel.find({ 
      productId: new Types.ObjectId(productId), 
      tenantId 
    })
      .populate("warehouseId", "name code city isActive")
      .lean() as unknown as Promise<IStockLevel[]>;
  }
  ...
  async getMovements(
    ...
    const [data, total] = await Promise.all([
      StockMovement.find(filter)
        ...
        .skip(skip)
        .limit(limit)
        .lean() as unknown as IStockMovement[],
  ...
  async createTransfer(
    ...
    const enriched: Omit<ITransferItem, '_id'>[] = [];
    for (const item of input.items) {
      ...
      const sl = await StockLevel.findOne({
        ...
        warehouseId: new Types.ObjectId(input.fromWarehouseId),
        tenantId,
      });
    ...
    const transfer = await StockTransfer.create({
      ...
      fromWarehouseId: new Types.ObjectId(input.fromWarehouseId),
      toWarehouseId: new Types.ObjectId(input.toWarehouseId),
      items: enriched,
      status: "pending",
      notes: input.notes,
  ...
  async getTransfers(
    ...
    if (query.fromWarehouseId) filter.fromWarehouseId = new Types.ObjectId(query.fromWarehouseId);
    if (query.toWarehouseId) filter.toWarehouseId = new Types.ObjectId(query.toWarehouseId);
    Object.assign(filter, buildDateRangeQuery(query.from, query.to));
    ...
    const [data, total] = await Promise.all([
      StockTransfer.find(filter)
        ...
        .skip(skip)
        .limit(limit)
        .lean() as unknown as IStockTransfer[],
  ...
  async approveTransfer(
    ...
    if (!updated) throw new NotFoundError("Transfer");
    emitToTenant(tenantId, "stock_transfer", {
      type: "approved",
      id,
  ...
  async dispatchTransfer(
    ...
    try {
      ...
      await StockTransfer.findByIdAndUpdate(
        ...
        {
          status: "in_transit",
          dispatchedBy: new Types.ObjectId(userId),
  ...
  async receiveTransfer(
    ...
    try {
      ...
      await StockTransfer.findByIdAndUpdate(
        id,
        { status: newStatus, receivedBy: new Types.ObjectId(userId), receivedAt: new Date() },
        { session },
      );
      await session.commitTransaction();
  ...
  async cancelTransfer(
    ...
    const updated = await StockTransfer.findByIdAndUpdate(
      id,
      { status: "cancelled", rejectionReason: reason, updatedBy: new Types.ObjectId(userId) },
      { new: true },
    );
    if (!updated) throw new NotFoundError("Transfer");
    return updated;
